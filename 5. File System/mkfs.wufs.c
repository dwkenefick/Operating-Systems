...
void buildInodes(void)
{
  ...
  int bblock = SB->sb_first_block; /* first possible bad block */
	...
	//First, we have to get rid of the first 7 blocks. after that, we can crap them into 
	//"special" inodes with no metadata. 
	

	//we want to do this for the first 7 blocks, and then a slightly differernt process 
	//for the next few 30's
	if (bbc){
		...
		while (bbc && (n < WUFS_INODE_BPTRS)) {
			...
			if (bblock == -1) {
				fprintf(stderr,"Infernal error: lost bad block while building root directory.\n");
				exit(1);
			}
		...
		//initialize the next part of the loop, if needed. 
		if(bbc){
			int *originalsize = &(bbino->size);
			int *link = &(bbino->next);		//a link to the past (inode)
			struct wufs_inode_shell bbinos;	
		}

		//now, if there are still bad blocks, we add new inode shells, each with 30 bloks avalible.
		while(bbc){
			bbinonum = allocInode();
			*link = bbinonum;
			bbinos = &Inode[bbinonum-1];
			link = &(bbinos->ins_next);			

			n=0;

			while (bbc && (n < WUFS_INODE_SIZE-2)) {
				bblock = findNextSet(BMap,bblock,SB->sb_blocks);
				/* sanity check: shouldn't run out of bad blocks before bbc hits zero */
				if (bblock == -1) {
					fprintf(stderr,"Infernal error: lost bad block while building root directory.\n");
				...
				if (bblock != rootDir) {
					...
					bbinos->in_block[n++] = bblock;
					*originalsize += WUFS_BLOCKSIZE;
					bbc--;
				}
				bblock++;
			}
